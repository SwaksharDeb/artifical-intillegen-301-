...
class Puzzle(Node):
    ...
    def f(self,start,goal):
        ...
        function_value = start.level+((self.h(start.data,goal)+self.mhd(start.data,goal)+self.euclidean_distance(start.data,goal))/3)
    ...
    def process(self):
        ...
        goal = self.accept()
        solution = self.check_solvability(goal)
        if solution == False:
            print("Not solvable: ")
            return 0
        ...
        while True:
            ...
            f1.write("local cost of above state is: ")
            f1.write(str(cur.level))
            f1.write("\n")
            print("hreustic value of above state is: ",((self.h(cur.data,goal)+self.mhd(cur.data,goal))/2))
            f1.write("hreustic value of above state is: ")
            f1.write(str((self.h(cur.data,goal)+self.mhd(cur.data,goal))/2))
            f1.write("\n")
            print("functional value of above state is: ",self.f(cur,goal))
            f1.write("functional value of above state is: ")
            f1.write(str(self.f(cur,goal)))
            f1.write("\n")
